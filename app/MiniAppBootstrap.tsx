...
export default function MiniAppBootstrap(){
  ...
  const [choices,setChoices]=useState<FamilyChoice[]>([]);const[error,setError]=useState("");const[busy,setBusy]=useState(false);
  ...
  useEffect(()=>{
    ...
    if(!supported){setError("این نسخه بله از Mini App پشتیبانی کامل نمی‌کند. بله را به آخرین نسخه به‌روزرسانی کن.");return}
  ...
  return <div className="miniappBootstrapOverlay" role="dialog" aria-modal="true"><div className="premiumPanel" style={{maxWidth:420,width:"calc(100% - 28px)",padding:18}}><h2 style={{marginTop:0}}>Family Bot</h2>{error?<><p>{error}</p><button className="adminSave" onClick={()=>webApp?.openLink?.(`https://ble.ir/${botUsername}`,{try_instant_view:true})}>رفتن به بازو در بله</button></>:<><p>یکی از خانواده‌ها را انتخاب کن:</p><div style={{display:"grid",gap:8}}>{choices.map(f=><button className="adminSave" disabled={busy} key={f.id} onClick={()=>void bootstrap(f.id)}>{f.name}</button>)}</div></>}<button className="ghostCta" style={{marginTop:12}} onClick={()=>webApp?.close?.()}>بستن</button></div></div>
}
