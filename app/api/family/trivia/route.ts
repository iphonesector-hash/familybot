...
export async function POST(req:NextRequest){
  try{
    ...
    if(body.action==="start"){
      ...
      const pay=CHALLENGE_REWARDS.trivia;
      const ins=await supabase.from("game_sessions").insert({family_id:session.familyId,chat_id:session.chatId,game_type:"family_trivia",prompt,answer,options,reward_coins:pay.coins,expires_at:new Date(Date.now()+120000).toISOString()}).select("id,prompt,options,reward_coins,expires_at").single();if(ins.error)throw ins.error;return NextResponse.json({ok:true,result:ins.data});
    }
    if(body.action==="answer"){
      ...
      if(q.data.expires_at&&new Date(q.data.expires_at).getTime()<Date.now()){await supabase.from("game_sessions").update({status:"closed"}).eq("id",id);return NextResponse.json({ok:false,error:"trivia_expired"},{status:409})}
      if(String(option)!==String(q.data.answer)){
        await supabase.from("game_sessions").update({status:"closed"}).eq("id",id).eq("status","open");
        return NextResponse.json({ok:true,result:{correct:false,reward:{coins:0,cp:0},alreadyClaimed:false}});
      }
      const won=await supabase.from("game_sessions").update({status:"closed",winner_bale_user_id:session.userId}).eq("id",id).eq("status","open").select("id").maybeSingle();
      if(!won.data)return NextResponse.json({ok:true,result:{correct:true,reward:{coins:0,cp:0},alreadyClaimed:true}});
      const pay=challengeReward("trivia");
      const coin=await supabase.rpc("family_add_member_coins",{p_member_id:member.data.id,p_delta:pay.coins});if(coin.error)throw coin.error;
      const xp=await supabase.rpc("family_add_member_xp",{p_member_id:member.data.id,p_delta:pay.cp});if(xp.error)throw xp.error;
      await Promise.all([supabase.from("coin_ledger").insert({family_id:session.familyId,member_id:member.data.id,amount:pay.coins,reason:"family_trivia_win",reference_type:"game_session",reference_id:id}),supabase.from("activity_log").insert({family_id:session.familyId,member_id:member.data.id,activity_type:"game_win",xp_delta:pay.cp,metadata:{game:"family_trivia"}})]);
