...
export async function GET(req:NextRequest){try{const s=sessionFrom(req);if(!s)return NextResponse.json({ok:false,error:"unauthorized"},{status:401});const [spy,nameFamily,twenty]=await Promise.all([readSpyGames(s.familyId,s.userId),readNameFamilyGames(s.familyId,s.userId),readTwentyQuestionsGames(s.familyId,s.userId)]);return NextResponse.json({ok:true,data:{spy:spy.games,nameFamily:nameFamily.games,twenty:twenty.games,me:spy.me}})}catch(error){console.error("multiplayer read failed",error);return NextResponse.json({ok:false,error:"multiplayer_read_failed"},{status:500})}}
...
else return NextResponse.json({ok:false,error:"unknown_action"},{status:400});return NextResponse.json({ok:true,data})}catch(error){const message=error instanceof Error?error.message:"multiplayer_action_failed";const conflict=["spy_game_started","spy_needs_three_players","spy_not_active","name_family_started","name_family_needs_two_players","name_family_not_active","name_family_time_up","name_family_finished","twenty_game_closed","twenty_game_started","twenty_needs_two_players","twenty_not_active","twenty_question_limit","twenty_already_answered"].includes(message);console.error("multiplayer action failed",error);return NextResponse.json({ok:false,error:message},{status:conflict?409:400})}}
