...
export async function GET(req:NextRequest){try{const s=sessionFrom(req);if(!s)return NextResponse.json({ok:false,error:"unauthorized"},{status:401});const m=await memberId(s.familyId,s.userId);const last=await db().from("daily_spin_claims").select("claimed_at,reward_kind,reward_amount").eq("family_id",s.familyId).eq("member_id",m).order("claimed_at",{ascending:false}).limit(1).maybeSingle();if(last.error)throw last.error;const nextAt=last.data?new Date(new Date(last.data.claimed_at).getTime()+24*60*60*1000).toISOString():null;return NextResponse.json({ok:true,eligible:!nextAt||Date.now()>=new Date(nextAt).getTime(),nextAt,last:last.data,rewards:rewards.map(({kind,amount,label})=>({kind,amount,label}))})}catch(error){console.error("spin status failed",error);return NextResponse.json({ok:false,error:"spin_status_failed"},{status:500})}}
export async function POST(req:NextRequest){try{const s=sessionFrom(req);if(!s)return NextResponse.json({ok:false,error:"unauthorized"},{status:401});const m=await memberId(s.familyId,s.userId);const reward=pickReward();const result=await db().rpc("family_claim_daily_spin_atomic",{p_family_id:s.familyId,p_member_id:m,p_reward_kind:reward.kind,p_reward_amount:reward.amount});if(result.error)throw result.error;const row=result.data as {claimed?:boolean;nextAt?:string;kind?:string;amount?:number};if(!row?.claimed)return NextResponse.json({ok:false,error:"spin_cooldown",nextAt:row?.nextAt},{status:409});return NextResponse.json({ok:true,reward:{kind:reward.kind,amount:reward.amount,label:reward.label},nextAt:row.nextAt})}catch(error){console.error("daily spin failed",error);return NextResponse.json({ok:false,error:"spin_failed"},{status:500})}}
