...
export async function GET(req:NextRequest){try{const s=sessionFrom(req);if(!s)return NextResponse.json({ok:false,error:"unauthorized"},{status:401});return NextResponse.json({ok:true,data:await readCommunity(s.familyId,s.userId)})}catch(error){console.error("community read failed",error);return NextResponse.json({ok:false,error:"community_read_failed"},{status:500})}}
export async function POST(req:NextRequest){try{const s=sessionFrom(req);if(!s)return NextResponse.json({ok:false,error:"unauthorized"},{status:401});const body=await req.json();const action=String(body?.action||""),p=body?.payload||{};let data:unknown;
 if(action==="challenge.create")data=await createChallenge(s.familyId,s.userId,{title:p.title,description:p.description,type:p.type,target:Number(p.target||1),reward:Number(p.reward||0),endsAt:p.endsAt||null});
