...
export async function POST(req: NextRequest) {
  try {
    ...
    const founder = liveAdmin ? false : await isFamilyFounder(session.familyId, session.userId).catch(() => false);
    if (!liveAdmin && !founder) return NextResponse.json({ ok: false, error: "admin_required" }, { status: 403 });
    const token = createAdminSession({ familyId: session.familyId, chatId: session.chatId, userId: session.userId }, 15 * 60);
    return NextResponse.json({ ok: true, token, expiresIn: 900, founder }, { headers: { "cache-control": "no-store" } });
  } catch (error) {
