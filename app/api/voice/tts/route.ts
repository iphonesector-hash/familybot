...
export async function POST(req:NextRequest){
  try{
    ...
    if(!key||!voiceId)return new Response(JSON.stringify({error:"voice_not_configured"}),{status:503,headers:{"content-type":"application/json"}});
    ...
    if(!response.ok){
      ...
      return new Response(JSON.stringify({error:"tts_provider_failed"}),{status:502,headers:{"content-type":"application/json"}});
    }
