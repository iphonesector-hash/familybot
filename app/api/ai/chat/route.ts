...
async function explicitAction(message:string,session:ReturnType<typeof sessionFrom>){if(!session)return null;
  const task=message.match(/(?:یک\s+)?کار(?:\s+جدید)?[:：]?\s*(.+?)\s*(?:بساز|ثبت\s*کن)$/i);if(task){const title=task[1].trim();await createFamilyTask(session.familyId,session.userId,{title,rewardCoins:0});return`✅ کار «${title}» برای خانواده ثبت شد.`}
  ...
  const poll=message.match(/(?:نظرسنجی|رأی.?گیری)[:：]?\s*(.+?)\s+گزینه(?:‌| )?ها[:：]?\s*(.+?)\s*(?:بساز|ثبت\s*کن)$/i);if(poll){const question=poll[1].trim(),options=poll[2].split(/[،,|]/).map(x=>x.trim()).filter(Boolean);if(options.length<2)return"برای نظرسنجی حداقل دو گزینه بده؛ مثلاً «نظرسنجی شام کجا باشه گزینه‌ها: خونه، رستوران بساز».";await createFamilyPoll(session.familyId,session.userId,{question,options});return`📊 نظرسنجی «${question}» با ${fa(options.length)} گزینه ساخته شد.`}
  const normalized=normalizeFaNumber(message);const transferA=normalized.match(/(?:به\s+)(.+?)\s+(\d+)\s*سکه\s*(?:بده|منتقل\s*کن)$/i);const transferB=normalized.match(/(\d+)\s*سکه\s+(?:به\s+)(.+?)\s*(?:بده|منتقل\s*کن)$/i);if(transferA||transferB){const name=(transferA?.[1]||transferB?.[2]||"").trim();const amount=Number(transferA?.[2]||transferB?.[1]||0);try{const target=await findFamilyMemberByName(session.familyId,name);const result=await transferFamilyCoins(session.familyId,session.userId,{targetMemberId:target.id,amount});return`🪙 ${fa(result.amount)} Family Coin به ${result.target.name} منتقل شد.`}catch(error){const m=error instanceof Error?error.message:"transfer_failed";if(m==="member_not_found")return`عضوی با نام «${name}» پیدا نکردم. اسم نمایشی عضو رو دقیق‌تر بگو.`;if(m==="member_name_ambiguous")return`چند عضو با نام «${name}» پیدا کردم؛ اسم کامل‌تر رو بگو.`;if(m==="insufficient_coins")return"سکه کافی برای این انتقال نداری 🪙";throw error}}
  return null;
}
...
export async function POST(req:NextRequest){
  try{
    ...
    const base=process.env.AI_BASE_URL?.replace(/\/$/,"");const key=process.env.AI_API_KEY;const model=process.env.AI_MODEL;if(!base||!key||!model)return NextResponse.json({reply:dashboard?"من به داده‌های Family Bot وصل هستم 💜 فعلاً مدل زبانی تنظیم نشده، ولی اکشن‌های امن مثل ساخت کار/رویداد/نظرسنجی، انتقال سکه و پرسیدن تولدهای نزدیک فعالن.":"من آماده‌ام 💜 فعلاً کلید مدل هوش مصنوعی روی سرور تنظیم نشده.",demo:true});
