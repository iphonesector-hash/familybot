...
async function handleCallback(query: NonNullable<Update["callback_query"]>) {
  ...
  if (data.startsWith("quiz:")) {
    ...
    if (result.ok) {
      ...
      await sendMessage(chatId, `🎉 ${user.first_name || "آفرین"}! جواب درست بود و ${fmt(result.reward)} سکه بردی.`);
    } else {
...
export async function POST(req: NextRequest) {
  ...
  if (ctx && userId && settings) {
    ...
    const whitelisted = !admin && await isWhitelisted(ctx.family.id, userId).catch(() => false);
    privileged = admin || whitelisted;
