...
export async function POST(req:NextRequest){
  try{
    ...
    const init=validateBaleInitData(String(body?.initData||""));
    if(!init?.user?.id)return NextResponse.json({ok:false,error:"invalid_init_data"},{status:401});
    const supabase=db();
    const {data:members,error}=await supabase.from("members").select("id,family_id,bale_user_id,display_name,first_name,last_active_at,families(id,name,bale_chat_id)").eq("bale_user_id",init.user.id).order("last_active_at",{ascending:false});
    ...
    const requestedFamilyId=body?.familyId?String(body.familyId):"";
    const selected=(requestedFamilyId?rows.find((r:any)=>String(r.family_id)===requestedFamilyId):rows.length===1?rows[0]:null) as any;
    ...
    const chatId=Number(selected.families.bale_chat_id);
    const token=createFamilySession({familyId:String(selected.family_id),chatId,userId:init.user.id},60*60*12);
    const canManage=await isAdmin(chatId,init.user.id).catch(()=>false);
