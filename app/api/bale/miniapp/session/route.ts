...
import {extractBalePhotoUrl,isFamilyUpload} from "@/lib/avatarResolve";
...
export async function POST(req:NextRequest){
 try{
  ...
  if(photo&&!isFamilyUpload(selected.avatar_url))patch.avatar_url=photo;
  await supabase.from("members").update(patch).eq("id",selected.id).eq("family_id",selected.family_id);
  return NextResponse.json({ok:true,status:"ready",session:token,canManage,family:{id:selected.family_id,name:selected.families?.name||"خانواده",chatId},user:{...init.user,photo_url:photo||init.user.photo_url}});
