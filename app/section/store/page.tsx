...
import {cacheDashboardShell,patchCachedProfile,readUiSnapshot} from "@/lib/miniAppUiCache";

type Owned={id:string;item_id:string;item_name:string;item_kind:string};
type Profile={coins?:number;is_founder?:boolean;equipped_profile_item?:string|null};
...
export default function StorePage(){
 ...
 return <main className="appShell storePage">
  ...
  <Accordion title="دارایی‌های من" summary={inventory.length?`${fa(inventory.length)} مورد`:"هنوز چیزی خریداری نشده"} icon={<Icon name="store" size={18}/>} defaultOpen>
   {inventory.length?<div className="storeGrid" style={{marginTop:4}}>{inventory.map(i=>{const active=profile.equipped_profile_item===i.id;return <article className={`dashboardCard storeAssetCard${active?" equippedStoreItem":""}`} key={`${i.id}-${i.stackable?"mat":"own"}`}><StoreItemArt itemId={i.id} size={64} label={i.name}/><div><h2>{i.name}</h2><p>{i.stackable?`موجودی انبار: ${fa(i.qty)}`:active?"در حال استفاده":"آیتم یکتا"}</p>{i.kind==="profile"?<button className={active?"ghostCta":"primaryCta"} disabled={busy.startsWith("equip:")} onClick={()=>void equip(active?null:i.id)}>{busy===`equip:${i.id}`||busy==="equip:none"?"...":active?"برداشتن قاب":"استفاده روی پروفایل"}</button>:null}</div></article>})}</div>:<p style={{fontSize:12,color:"#c9bfd8"}}>بعد از خرید، آیتم‌ها و مصالح اینجا دیده می‌شوند.</p>}
  </Accordion>
  {STORE_GROUPS.map((g,idx)=>{const items=STORE_ITEMS.filter(g.match);if(!items.length)return null;const open=focus==="sagool"?g.id==="care"||g.id==="toys"||g.id==="accessories":focus==="profile"?g.id==="profile":idx<2;return <Accordion key={g.id} title={g.title} summary={`${g.summary} · ${fa(items.length)} آیتم`} defaultOpen={open}><div className="storeGrid" style={{marginTop:4}}>{items.map(i=>{const stacked=Boolean(i.stackable),have=own.has(i.id),stock=i.material?qty[i.material]||0:0,active=profile.equipped_profile_item===i.id;return <article className={`dashboardCard storeAssetCard rarity-${i.rarity}${active?" equippedStoreItem":""}`} key={i.id}><StoreItemArt itemId={i.id} size={72} label={i.name}/><div><h2>{i.name}</h2><p>{i.description}</p>{stacked&&<small>موجودی انبار: {fa(stock)}</small>}<b className="storePrice">{founder?"∞":`${fa(i.price)} سکه`}</b>{i.kind==="profile"&&have?<button className={active?"ghostCta":"primaryCta"} disabled={busy.startsWith("equip:")} onClick={()=>void equip(active?null:i.id)}>{active?"برداشتن قاب":"استفاده"}</button>:<button className="primaryCta" disabled={busy===i.id||(!stacked&&have)} onClick={()=>void buy(i.id)}>{!stacked&&have?"خریداری شده":busy===i.id?"...":founder?"دریافت":stacked?"خرید بسته":"خرید"}</button>}</div></article>})}</div></Accordion>})}
