...
export default function GamesPage(){
  const[d,setD]=useState<Dashboard>({});const[notice,setNotice]=useState("");const[busy,setBusy]=useState(false);const[quiz,setQuiz]=useState<Quiz|null>(null);
  ...
  async function play(action:string,extra:Record<string,unknown>={}){setBusy(true);setNotice("");try{const r=await api(action,extra);if(action==="dice")setNotice(`🎲 عدد تاس: ${fa(r.value)}`);if(action==="coin")setNotice(`🪙 نتیجه: ${r.side}`);if(action==="rps"){const names=["سنگ","کاغذ","قیچی"];setNotice(`تو: ${names[r.choice]} · ربات: ${names[r.bot]} — ${r.outcome==="win"?`بردی! +${fa(r.reward)} سکه 🎉`:r.outcome==="draw"?"مساوی شد 😄":"این دست ربات برد"}`)}if(action==="quiz.start")setQuiz(r)}catch(e){setNotice(e instanceof Error?e.message:"بازی اجرا نشد")}finally{setBusy(false)}}
  ...
  const leaders=d.leaderboard||[];const profile=d.profile;
  return <main className={styles.page}>
    ...
    <div className={styles.title}><h2>بازی‌های گروهی</h2><span>موتور بعدی</span></div>
    <section className={styles.gameGrid}>{[["⚔️","دوئل"],["🕵️","جاسوس"],["🏁","مسابقه سرعت"],["📝","اسم‌فامیل"],["🎭","مافیا Lite"],["❓","۲۰ سؤال"]].map(([icon,name])=><article className={styles.game} key={name}><span className={styles.gameIcon}>{icon}</span><h3>{name}</h3><p>گروهی · نیازمند هماهنگی چند عضو</p><b>در فاز Multiplayer بعدی</b></article>)}</section>
    ...
    <section className={styles.rewards}><header><h2>جایزه روزانه</h2><span>Streak: {fa(profile?.streak||0)} روز</span></header><p style={{fontSize:10,color:"#a99fba"}}>جایزه روزانه واقعی از منوی ربات یا صفحه پروفایل قابل Claim است؛ اینجا عدد نمونه نمایش داده نمی‌شود.</p></section>
