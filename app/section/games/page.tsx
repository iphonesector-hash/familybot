...
export default function GamesPage(){
  ...
  return <main className={styles.page}>
    ...
    <div className={styles.title}><h2>بازی‌های گروهی</h2><span>۱ فعال</span></div>
    <section className={styles.gameGrid}><Link href="/section/community" className={styles.game}><span className={styles.gameIcon}>⚔️</span><h3>دوئل خانوادگی</h3><p>دو نفره · انتخاب همزمان سنگ/کاغذ/قیچی</p><b>جایزه واقعی Family Coin</b><i>فعال</i></Link>{[["🕵️","جاسوس"],["🏁","مسابقه سرعت"],["📝","اسم‌فامیل"],["🎭","مافیا Lite"],["❓","۲۰ سؤال"]].map(([icon,name])=><article className={styles.game} key={name}><span className={styles.gameIcon}>{icon}</span><h3>{name}</h3><p>گروهی · موتور مستقل لازم دارد</p><b>در حال توسعه</b></article>)}</section>
