...
type Memory={id:string;title?:string|null;caption?:string|null;media_url?:string|null;memory_date?:string|null;tags?:string[];visibility?:"family"|"private"|"selected";created_at:string};
export default function MemoriesPage(){const [memories,setMemories]=useState<Memory[]>([]),[members,setMembers]=useState<Member[]>([]),[count,setCount]=useState(0),[live,setLive]=useState(false);
  const load=useCallback(()=>{const session=sessionStorage.getItem("familybot.session");if(!session)return;fetch("/api/family/dashboard",{headers:{authorization:`Bearer ${session}`},cache:"no-store"}).then(r=>r.json()).then(x=>{if(x.ok&&x.dashboard){setMemories(x.dashboard.memories||[]);setMembers(x.dashboard.members||[]);setCount(x.dashboard.family?.memoriesCount||0);setLive(true)}}).catch(()=>undefined)},[]);useEffect(()=>load(),[load]);
