...
export async function baleApi<T = unknown>(method: BaleMethod, payload: Record<string, unknown> = {}) {
  ...
  const response = await fetch(`${API_BASE}${token}/${method}`, {method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify(payload),cache:"no-store"});
  const data = await response.json();
  if (!response.ok || data?.ok === false) throw new Error(data?.description ?? `Bale API ${method} failed`);
  return data as T;
}
...
function webAppButton(base:string|undefined,path:string,text:string,fallback:string):BaleInlineButton{return base?{text,web_app:{url:new URL(path,base).toString()}}:{text,callback_data:fallback}}
export function mainMenuKeyboard(canManage=false){
  ...
  const treeButton=webAppButton(base,"/section/tree","🌳 شجره‌نامه","menu:tree");
  const rows:BaleInlineButton[][]=[
    [{text:"🏠 Mini App",callback_data:"menu:miniapp"}],
    [{text:"👨‍👩‍👧‍👦 خانواده",callback_data:"menu:family"},treeButton],
    [{text:"📅 برنامه‌ریز",callback_data:"menu:planner"},wheelButton],
    [fundButton,financeButton],
    [{text:"🖼 خاطرات",callback_data:"menu:memories"},communityButton],
    [secretGiftButton,{text:"🛍 فروشگاه",callback_data:"menu:store"}],
    [{text:"🏅 دستاوردها",callback_data:"menu:achievements"},{text:"🎮 بازی‌ها",callback_data:"menu:games"}],
