...
export async function readPlannerData(familyId:string){
  ...
  const [pollsRes,eventsRes]=await Promise.all([
    supabase.from("polls").select("id,question,options,anonymous,closes_at,created_at").eq("family_id",familyId).order("created_at",{ascending:false}).limit(10),
    supabase.from("family_events").select("id,title,description,event_type,starts_at,ends_at,location_text,created_at").eq("family_id",familyId).order("starts_at",{ascending:true}).limit(20),
  ]);
  ...
  const rich=polls.map(p=>{const options=Array.isArray(p.options)?p.options.map(String):[];const own=votes.filter(v=>v.poll_id===p.id);const counts=options.map((_,i)=>own.filter(v=>v.option_index===i).length);const total=counts.reduce((a,b)=>a+b,0);return{...p,options,results:counts.map((count,i)=>({option:options[i],count,percent:total?Math.round(count/total*100):0})),totalVotes:total}});
