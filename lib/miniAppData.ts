...
export async function readMiniAppDashboard(familyId:string,userId:number){
  ...
  const [familyRes,profileRes,membersRes,allMembersRes,leaderboardRes,birthdaysRes,tasksRes,eventsRes,memoriesCountRes,memoriesListRes,relationsRes,ownedRes]=await Promise.all([
    ...
    supabase.from("memories").select("id,title,caption,media_url,memory_date,tags,created_at").eq("family_id",familyId).order("memory_date",{ascending:false,nullsFirst:false}).limit(24),
  ...
  for(const result of [familyRes,profileRes,membersRes,allMembersRes,leaderboardRes,birthdaysRes,tasksRes,eventsRes,memoriesCountRes,memoriesListRes,relationsRes,ownedRes])if(result.error)throw result.error;
  ...
  return{
    family:{id:family.id,name:family.name,level:levelBase,xp:familyXp,coins:Number(family.coins||0),houseLevel:Math.max(Number(family.house_level||1),levelBase),membersCount:membersRes.count??0,upcomingEventsCount:eventsRes.data?.length??0,upcomingBirthdaysCount:birthdays.filter(b=>b.days<=30).length,memoriesCount:memoriesCountRes.count??0,levelProgress:{current:Math.max(0,familyXp-levelFloor),target:Math.max(1,levelCeil-levelFloor)}},
    profile:profileRes.data?{...profileRes.data,rank}:null,
    members:allMembers,
    leaderboard,birthdays,tasks:tasksRes.data??[],events:eventsRes.data??[],memories:memoriesListRes.data??[],relationships:relationsRes.data??[],ownedItems:ownedRes.data??[],generatedAt:now,
  };
}
