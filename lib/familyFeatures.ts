...
export async function transferFamilyCoins(familyId:string,userId:number,input:{targetUserId?:number;targetMemberId?:string;amount?:number;note?:string}){
  const supabase=db();const sender=await member(familyId,userId);const amount=Math.max(1,Math.min(100000,Math.floor(Number(input.amount)||0)));if(!amount)throw new Error("invalid_amount");let targetQuery=supabase.from("members").select("id,bale_user_id,display_name,first_name,coins").eq("family_id",familyId);targetQuery=input.targetMemberId?targetQuery.eq("id",input.targetMemberId):targetQuery.eq("bale_user_id",Number(input.targetUserId));const target=await targetQuery.single();if(target.error)throw target.error;if(target.data.id===sender.id)throw new Error("cannot_transfer_to_self");const senderCoins=Number(sender.coins||0);if(senderCoins<amount)throw new Error("insufficient_coins");
  const note=String(input.note||"").slice(0,200)||null;const s=await supabase.from("members").update({coins:senderCoins-amount}).eq("id",sender.id).eq("coins",senderCoins).select("id").maybeSingle();if(!s.data)throw new Error("coin_balance_changed");const receiverCoins=Number(target.data.coins||0);const r=await supabase.from("members").update({coins:receiverCoins+amount}).eq("id",target.data.id);if(r.error){await supabase.from("members").update({coins:senderCoins}).eq("id",sender.id);throw r.error}
  await supabase.from("coin_ledger").insert([{family_id:familyId,member_id:sender.id,amount:-amount,reason:"coin_transfer_out",reference_type:"member",reference_id:target.data.id},{family_id:familyId,member_id:target.data.id,amount,reason:"coin_transfer_in",reference_type:"member",reference_id:sender.id}]);return{amount,note,senderCoins:senderCoins-amount,target:{id:target.data.id,bale_user_id:target.data.bale_user_id,name:target.data.display_name||target.data.first_name||"عضو خانواده"}};
}
...
export async function evaluateAchievements(familyId:string,userId:number){
  ...
  await supabase.from("achievements").upsert(ACHIEVEMENTS.map(a=>({id:a.id,title:a.title,description:a.description,icon:a.icon,reward_coins:a.reward})),{onConflict:"id"});const owned=await supabase.from("member_achievements").select("achievement_id").eq("member_id",m.id);if(owned.error)throw owned.error;const ownedSet=new Set((owned.data||[]).map(x=>x.achievement_id));let reward=0;const unlocked:string[]=[];for(const id of eligible){if(ownedSet.has(id))continue;const a=ACHIEVEMENTS.find(x=>x.id===id)!;const ins=await supabase.from("member_achievements").insert({member_id:m.id,achievement_id:id});if(!ins.error){reward+=a.reward;unlocked.push(id);await supabase.from("coin_ledger").insert({family_id:familyId,member_id:m.id,amount:a.reward,reason:"achievement_unlock",reference_type:"achievement",reference_id:id})}}
  if(reward)await supabase.from("members").update({coins:Number(m.coins||0)+reward}).eq("id",m.id);return{unlocked,reward,catalog:ACHIEVEMENTS};
}
