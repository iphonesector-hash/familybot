...
export async function evaluateAchievements(familyId:string,userId:number){
  ...
  await supabase.from("achievements").upsert(ACHIEVEMENTS.map(a=>({id:a.id,title:a.title,description:a.description,icon:a.icon,reward_coins:a.reward})),{onConflict:"id"});const owned=await supabase.from("member_achievements").select("achievement_id").eq("member_id",m.id);if(owned.error)throw owned.error;const ownedSet=new Set((owned.data||[]).map(x=>x.achievement_id));let reward=0;const unlocked:string[]=[];for(const id of eligible){if(ownedSet.has(id))continue;const claim=await supabase.rpc("family_claim_achievement_atomic",{p_family_id:familyId,p_member_id:m.id,p_achievement_id:id});if(claim.error)throw claim.error;const row=claim.data as {claimed?:boolean;reward?:number}|null;if(row?.claimed){reward+=Number(row.reward||0);unlocked.push(id);ownedSet.add(id)}}
  return{unlocked,reward,owned:[...ownedSet],catalog:ACHIEVEMENTS};
}
