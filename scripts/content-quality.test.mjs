...
assert.ok(jokes.length>=150,"curated joke pool must contain at least 150 distinct items");
assert.equal(new Set(jokes.map(x=>x.text.replace(/\s+/g," ").trim())).size,jokes.length,"jokes must be text-distinct");
...
const {loader}=await import("./phase2-content-probe.mjs");
const loaded=loader()("lib/dezfuliCulture.ts");
const words=loaded.DEZFULI_WORDS.length,proverbs=loaded.DEZFULI_PROVERBS.length,poems=loaded.DEZFULI_POEMS.length;
assert.ok(words>=50);assert.ok(proverbs>=20);assert.ok(poems>=3);
