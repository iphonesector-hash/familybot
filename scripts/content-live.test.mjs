...
assert.match(load("lib/contentGanjoor.ts"),/api.ganjoor.net/);
assert.match(remote,/wikipedia-fa/);
assert.doesNotMatch(remote,/wikiquote-fa/);
assert.doesNotMatch(remote,/wikipedia-fa-humor/);
...
assert.match(remote,/JOKE_SOURCE_ORDER=\["reviewed-pool","sector-ai","curated-local"\]/);
assert.match(remote,/RIDDLE_SOURCE_ORDER=\["wikipedia-fa-chistan","wiki-chistan-import","curated-local"\]/);
...
const photo=load("lib/baleProfilePhoto.ts");
assert.doesNotMatch(photo,/baleCall\("getUserProfilePhotos"/);
assert.match(photo,/familybot-avatars/);
assert.match(photo,/download_invalid_mime/);
const avatar=load("app/ui/Avatar.tsx");
assert.match(avatar,/setBroken\(false\)/);
const css=load("app/premium-ui.css");
assert.match(css,/--visual-vh/);
assert.match(css,/keyboardOpen/);
assert.doesNotMatch(css,/translateY\(var\(--vv-offset-top/);
